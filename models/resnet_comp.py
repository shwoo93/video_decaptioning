...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        # DECODER-MASK
        self.mslayer1 = nn.Sequential(
                        nn.Conv3d(512,256,kernel_size=(3,3,3), stride=(1, 1, 1), padding=(1, 1, 1), bias=False),
                        nn.BatchNorm3d(256),
                        nn.ReLU(inplace=True)
                        ) #512X2x16x16
        self.mslayer2 = nn.Sequential(
                        nn.Conv3d(256,128,kernel_size=(3,3,3), stride=(1, 1, 1), padding=(1, 1, 1), bias=False),
                        nn.BatchNorm3d(128),
                        nn.ReLU(inplace=True)
                        ) #256x4x32x32
        self.mslayer3 = nn.Sequential(
                        nn.Conv3d(128,64,kernel_size=(3,3,3), stride=(1, 1, 1), padding=(1, 1, 1), bias=False),
                        nn.BatchNorm3d(64),
                        nn.ReLU(inplace=True)
                        ) #128x8x64x64
        self.mslayer4 = nn.Sequential(
                        nn.Conv3d(64,64,kernel_size=(3,3,3), stride=(1, 1, 1), padding=(1, 1, 1), bias=False),
                        nn.BatchNorm3d(64),
                        nn.ReLU(inplace=True)
                        ) #64x16x128x128
        self.msconv1 = nn.Sequential(
                       nn.Conv3d(64,1,kernel_size=3, stride=(1,1,1), padding=(1,1,1), bias=False),
                       nn.Sigmoid()
                       )
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        # print('L3',x.size())
        encoded = self.layer4(x)
        # print('L4',x.size())
        x = self.unlayer1(F.upsample(encoded, size=(self.sample_duration,16,16), mode='trilinear'))
        ...
        x = self.unlayer4(F.upsample(x, size=(self.sample_duration,128,128), mode='trilinear'))
        out = self.unconv1(x)

        x = self.mslayer1(F.upsample(encoded, size=(self.sample_duration,16,16), mode='trilinear'))
        x = self.mslayer2(F.upsample(x, size=(self.sample_duration,32,32), mode='trilinear'))
        x = self.mslayer3(F.upsample(x, size=(self.sample_duration,64,64), mode='trilinear'))
        x = self.mslayer4(F.upsample(x, size=(self.sample_duration,128,128), mode='trilinear'))
        mask = self.msconv1(x)

        comp = input*(1-mask) + out*mask
        # return x
        return comp, mask
